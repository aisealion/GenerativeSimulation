# Phase 1 specification for round 4

This specification documents the implementation of the norm for round 4 as described in `norm.txt`.  The norm enforces a 20 kg per‑trip cap, adds any excess catch to a communal reserve, and imposes a one‑month fishing ban on agents who exceed the cap.
...
- Implements:
  ...
  - `evaluate` – if `raw_kg` ≤ 20 kg, allow catch; otherwise add full `raw_kg` to communal reserve (`norm_state[self.RESERVE_KEY]`) and set a 1‑month ban, returning a `NormDecision.violation` with `kept_kg=0.0`.
...
## Expected behavior
1. Agents catching ≤ 20 kg keep their catch.
2. Agents catching > 20 kg forfeit the entire catch to the communal reserve, receive a note explaining the forfeiture, and are banned for the next round.
...
Write this specification to `state/norm_specs/round_4.md`.
