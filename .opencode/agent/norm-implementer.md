...
- **`new_phase`, or any fragment needing `phases/` or `mechanisms/`
  ...
  temporarily widen `permission.edit` before it can be attempted. This is
  the same "stop rather than guess" posture as an ambiguous parameter,
  just for a different reason (permission, not information).
...
norm-implementer file — not on your allowlist, denied outright by this
file's own `permission.edit`. `engine/simulate.py` is allowed but last
resort only: reserve it for genuinely orchestration-level changes (a new
