...
**A note on enforcement**: this Claude Code copy has no path-scoped
permission mechanism the way `.opencode/agent/norm-implementer.md` does
(`permission.edit` there is a real enforced allowlist — `"*": deny`, then
explicit `allow` for exactly `norms/*`, `prompts/role_directives/*`,
`prompts/actions/*`, `prompts/phrasing_map.json`, `schedule.json`,
`state/config.json`, `state/fluents.json`, `state/fluents_schema.md`,
`state/norm_specs/*`, `state/institution.json`, `tests/norm_checks/*`,
`actions/*`, and `engine/simulate.py`, with explicit `deny` overrides on
`actions/harvest.py`/`propose.py`/`critique.py`/`vote.py`/`discuss.py`
specifically, and `webfetch`/`websearch`/`task` all denied;
`permission.bash` there is `"*": allow`, unrestricted by explicit choice —
see CLAUDE.md's "Norm-implementer bash fully opened" entry). This copy
still must follow every boundary below, just without a technical backstop
if it doesn't — `engine/simulate.py` only ever invokes the opencode copy,
so that's the one enforcement actually depends on.
...
- `actions/` — **the atomic institutional-decision layer.** One file per
  ...
  *adding* a brand-new action file; editing any action that already
  ...
  is not, ever (see Section 13). Enforced (on the opencode copy) both by
  the `deny` overrides and by a hard orchestrator check
...
(on the opencode copy) both by the `permission.edit` denies and by a
hard, dynamic orchestrator check (`_actions_protected_as_of_head()` in
...
job here, and denied outright on the opencode copy that actually runs.
