...
- **new_phase, no existing phase fits (confirmed via Step 2)**: add one file
  ...
  `engine/simulate.py` is schedule-driven, not hardcoded — it imports and
  calls every phase listed in `schedule.json` whose gate is currently
  true, in the file's key order, every round. This is a real contract, not
  just documentation:
  - Your phase module must define a `Phase` subclass (import `Phase` from
    `engine.phase_base`) and a module-level `PHASE = YourPhaseSubclass()`
    instance — `engine/simulate.py` calls `PHASE.run(state) -> dict`.
    `state` has `config`, `fluents`, `runtime`, `agents`, `round_number` —
    mutate `runtime`/`fluents` in place as needed; the returned dict is
    appended to `runtime["rounds"]` for history/logging. If the phase
    calls the fisher agent, also implement
    `prompt_fields(state, agent_id) -> dict` with the fields
    `prompts/phases/{phase_name}.md` needs — `run()` calls it per agent
    rather than inlining the field values.
  ...
  - If your phase is the one that decides a norm (an alternative to
    ...
    before returning. `engine/simulate.py` checks for that key after all of a
...
finishing. An incomplete-but-reported round is recoverable —
`engine/simulate.py`'s compile-check will catch a half-finished edit the
same as any other broken one, and the round gets retried — a silent
cutoff with no report is not: there's nothing for the orchestrator or a
human reviewing `model_calls.jsonl` later to distinguish it from an
unexplained crash.
...
- Never edit `state/runtime.json`, `state/agents.json`,
  ...
  specific `prompts/` files named above. The
  `.opencode/agent/norm-implementer.md` copy of this file enforces this
  list technically via `permission.edit` rules — a `"*": deny` default
  with explicit `allow` entries for exactly those paths, replacing an
  earlier allow-everything-except-a-denylist model after a real incident
  where a `deny` entry did not actually stop an edit from landing on disk.
  `permission.bash` there is scoped the same way (`"*": deny`, then
  allowlisted patterns for `python3 -m py_compile`, `pytest`, read-only
  `git status`/`diff`/`log`, `codegraph`, `grep`) rather than left as a
  blanket allow — an unscoped bash is a direct bypass of the edit
  allowlist (`echo ... > file` accomplishes what `edit: deny` blocks), and
  the fallback there is `deny` rather than `ask` specifically because this
  agent always runs via `opencode run` with nobody present to answer a
  prompt, and opencode has a known issue where an unanswered `ask` on bash
  hangs rather than failing. `webfetch`, `websearch`, and `task` (subagent
  spawning) are denied there too — nothing in this job needs any of them.
  This Claude Code copy doesn't support the same path-scoped mechanism,
  but `engine/simulate.py` only ever invokes the opencode agent, so that's
