...
both by the permission denies (on the opencode copy) and by a hard
orchestrator check (`norm_implementation_protected_path_violations()` in
...
which is exactly why the protected-phase-file guarantee above rests on the
orchestrator's own `git diff` check, not on the `deny` overrides actually
holding.
...
brand new `phases/{name}.py` file is allowed (see PHASE 3's `new_phase`
recipe) — the distinction is add vs. edit, not "phases/ is off-limits"
anymore. `engine/simulate.py` is allowed but last resort only: reserve it
for genuinely orchestration-level changes (a new scheduling primitive, a
cross-phase safety check) — never a convenient place to patch a bug that
actually belongs in a norm plugin's own logic.
